...
- **Characters**: List + detail view with editable 5e character sheets + multi-step creation wizard (basics → ability scores → combat → details). Players land directly on a "My Character" tab at login if they own a character (otherwise see a Create-your-character CTA); DMs land on Overview as before. Owners and the DM can download or print a Delve-styled 5e character sheet PDF (generated server-side with `pdf-lib`, `GET /characters/:id/pdf`, `?download=1` forces an attachment).
