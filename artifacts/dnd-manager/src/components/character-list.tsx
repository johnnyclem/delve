        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {(characters as Character[]).map((char) => (
            <button
              ...
              className="text-left rounded-xl border border-border/50 bg-card p-5 hover:border-primary/30 transition-colors"
              data-testid={`card-character-${char.id}`}
            >
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold text-card-foreground">{char.name}</h3>
...
function CharacterDetail({ id, onBack }: { id: number; onBack: () => void }) {
  ...
  return (
    <div className="space-y-6" data-testid="character-detail">
      ...
      <div>
        <h2 className="font-serif text-2xl font-bold text-foreground" data-testid="text-character-name">{char.name}</h2>
        <p className="text-muted-foreground">
          Level {char.level} {char.race} {char.class} — played by {char.ownerDisplayName}
        </p>
      </div>
      ...
      {sheet && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="rounded-xl border border-border/50 bg-card p-5">
            ...
            <div className="flex items-baseline gap-1">
              {editing && editSheet ? (
                ...
                <span className="font-mono text-2xl font-bold text-foreground" data-testid="text-current-hp">{sheet.currentHp ?? 0}</span>
              )}
              <span className="text-muted-foreground text-sm">/ {sheet.maxHp ?? 0}</span>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border/50 bg-card p-5">
            ...
            <span className="font-mono text-2xl font-bold text-foreground" data-testid="text-armor-class">{sheet.armorClass ?? 10}</span>
          </div>
          ...
          <div className="rounded-xl border border-border/50 bg-card p-5">
            ...
            <span className="font-mono text-2xl font-bold text-foreground">{sheet.speed ?? 30} ft</span>
          </div>
                  ...
                  <div key={stat} className="text-center p-2 rounded-lg bg-muted/50">
                    ...
                    {editing && editSheet ? (
                      <Input
                        ...
                        className="w-14 mx-auto text-center font-mono text-sm"
                        data-testid={`input-${stat}`}
                      />
                    ) : (
                      <p className="font-mono text-lg font-bold text-foreground" data-testid={`text-${stat}`}>{val}</p>
                    )}
            ...
            <div className="rounded-xl border border-border/50 bg-card p-5 md:col-span-2 lg:col-span-3">
              ...
              <div className="flex flex-wrap gap-2">
                {sheet.inventory.map((item: string, i: number) => (
                  <span key={i} className="px-2 py-1 bg-muted/50 rounded text-xs text-foreground">{item}</span>
                ))}
              </div>
            </div>
          )}
